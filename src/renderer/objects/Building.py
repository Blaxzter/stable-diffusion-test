...
class Building:
    ...
    def trim_to_size(self):
        ...
        self.voxels = self.voxels[x_min:x_max+1, y_min:y_max+1, z_min:z_max+1]
    ...
    def create_door(self):
        ...
        if self.config.orientation == Orientation.NORTH:
            ...
            self.voxels[door_pos-1:door_pos+1, 1:self.config.door_height+1, p:p+1] = Material.DOOR
        elif self.config.orientation == Orientation.SOUTH:
            door_pos = self.config.length // 2 + p
            self.voxels[door_pos-1:door_pos+1, 1:self.config.door_height+1, -p-1:-p] = Material.DOOR
        elif self.config.orientation == Orientation.EAST:
            door_pos = self.config.width // 2 + p
            self.voxels[-p-1:-p, 1:self.config.door_height+1, door_pos-1:door_pos+1] = Material.DOOR
        elif self.config.orientation == Orientation.WEST:
            door_pos = self.config.width // 2 + p
            self.voxels[p:p+1, 1:self.config.door_height+1, door_pos-1:door_pos+1] = Material.DOOR

    def create_windows(self):
        p = self.padding
        window_spacing = 3

        def create_window_row(start_idx, end_idx, height_range, wall_idx, is_length_wall=True):
            """Helper function to create a row of windows"""
            window_positions = np.arange(start_idx + window_spacing, end_idx - window_spacing, window_spacing * 2)
            for pos in window_positions:
                if is_length_wall:
                    self.voxels[pos+p:pos+p+self.config.window_size,
                    height_range[0]:height_range[1],
                    wall_idx] = Material.WINDOW
                else:
                    self.voxels[wall_idx,
                    height_range[0]:height_range[1],
                    pos+p:pos+p+self.config.window_size] = Material.WINDOW

        # Window height range
        height_range = (self.config.window_height,
        ...
        if self.config.height >= 7:
            second_height_range = (self.config.height - 2,
                                   self.config.height - 1)

            create_window_row(0, self.config.length, second_height_range, self.padding, True)
            create_window_row(0, self.config.length, second_height_range, -self.padding-1, True)
            create_window_row(0, self.config.width, second_height_range, self.padding, False)
    ...
    def rotate(self, new_orientation: Orientation):
        ...
        if abs(new_orientation.value - self.config.orientation.value) % 2 == 1:
            self.voxels = np.zeros((self.config.width + 1 + (self.padding * 2),
                                    self.config.height + self.config.roof_height + 1 + self.padding,
                                    self.config.length + 1 + (self.padding * 2)), dtype=np.int8)
            # Swap length and width in config
            self.config.length, self.config.width = self.config.width, self.config.length

...
class World:
    ...
    def add_object(self, name: str, building: Building) -> bool:
        ...
        if (x + obj_shape[0] > self.world_size[0] or
                y + obj_shape[1] > self.world_size[1] or
                z + obj_shape[2] > self.world_size[2]):
            print(f"Object {name} doesn't fit in the world at position {building.config.position}")
            return False

        # Check if space is already occupied
        if np.any(self.voxels[x:x + obj_shape[0],
                  y:y + obj_shape[1],
                  z:z + obj_shape[2]] != Material.AIR):
            print(f"Space for object {name} is already occupied")
            return False

        # Add object to world
        self.voxels[x:x + obj_shape[0],
        y:y + obj_shape[1],
        z:z + obj_shape[2]] = building.voxels
